...
pub enum VaultActions {
    ...
    Initialize {
        /// The keeper used to create the VRT mint
        vrt_mint_keypair_path: String,
...
impl VaultCliHandler {
    ...
    pub async fn initialize_vault(
        ...
        let vrt_mint = Keypair::read_from_file(Path::new(&vrt_mint_keypair_path)).unwrap();
    ...
    pub async fn create_token_metadata(
        ...
        let (metadata_account_pubkey, _) = find_metadata_account(&vault_account.vrt_mint);
        ...
        ix_builder
            ...
            .mpl_token_metadata_program(jito_vault_sdk::inline_mpl_token_metadata::id())
        ...
        info!(
            "Create token metadata transaction: {:?}",
            tx.get_signature()
        );
    ...
    pub async fn set_capacity(
        ...
        let mut ix_builder = SetDepositCapacityBuilder::new();
        ix_builder
            .config(Config::find_program_address(&self.vault_program_id).0)
            .vault(vault)
            .admin(keypair.pubkey())
    ...
    pub async fn set_secondary_admin(
        ...
        let role = match role.as_str() {
            "DelegationAdmin" => VaultAdminRole::DelegationAdmin,
            "OperatorAdmin" => VaultAdminRole::OperatorAdmin,
            "NcnAdmin" => VaultAdminRole::NcnAdmin,
            "SlasherAdmin" => VaultAdminRole::SlasherAdmin,
            "CapacityAdmin" => VaultAdminRole::CapacityAdmin,
            "FeeWallet" => VaultAdminRole::FeeWallet,
            "MintBurnAdmin" => VaultAdminRole::MintBurnAdmin,
            "WithdrawAdmin" => VaultAdminRole::WithdrawAdmin,
            "FeeAdmin" => VaultAdminRole::FeeAdmin,
            _ =>return Err(anyhow::anyhow!("Unknown role"))
        };
        ...
        info!(
            "Set secondary admin ({:?}) transaction: {:?}",
            role,
            tx.get_signature()
        );
